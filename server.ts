...
async function apisyriaRequest(params: Record<string, string>): Promise<any> {
  ...
  const res = await fetch(`${APISYRIA_BASE}?${qs}`, {
    headers: { "Accept": "application/json" }
  });
  return res.json();
}

async function verifySyriatelTx(txNumber: string, gsm: string): Promise<{ found: boolean; amount?: number }> {
  try {
...
async function verifyShamCashTx(txNumber: string, accountAddress: string): Promise<{ found: boolean; amount?: number; currency?: string }> {
  try {
  ...
  app.post("/api/transactions/verify-auto", async (req, res) => {
    try {
      ...
      const { data: method } = await supabase.from("payment_methods")
        .select("name, method_type, api_account, min_amount")
      ...
      if (method.method_type === "syriatel") {
        const result = await verifySyriatelTx(txNumber, method.api_account);
        verified = result.found;
        apiAmount = result.amount || 0;
      } else if (method.method_type === "shamcash") {
        const result = await verifyShamCashTx(txNumber, method.api_account);
        verified = result.found;
        apiAmount = result.amount || 0;
      }

      if (!verified) {
        return res.status(400).json({ error: "لم يتم العثور على العملية. تأكد من رقم العملية أو انتظر قليلاً وأعد المحاولة." });
      }
      ...
      const { data: tx, error: txErr } = await supabase.from("transactions").insert({
        ...
        tx_number: txNumber,
        status: "approved",
        note: `تحقق تلقائي - رقم العملية: ${txNumber}`
      }).select().single();
      if (txErr) throw txErr;

      // Add balance to user
      const newBalance = (parseFloat(String(user.balance)) || 0) + numAmount;
