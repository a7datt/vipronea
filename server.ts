...
async function startServer() {
  ...
  app.post("/api/orders", async (req, res) => {
    try {
      ...
      const unitPrice = product.store_type === 'quantities'
  ...
  app.post("/api/admin/products", async (req, res) => {
    try {
      let {
        ...
        min_quantity, available, external_id
      } = req.body;

      // البحث بـ special_id إذا لم يُرسل id مباشرة
      if (!subcategory_id && subcategory_special_id) {
        let query = supabase.from("subcategories").select("id").eq("special_id", subcategory_special_id);
        if (category_special_id) {
          const { data: cat } = await supabase.from("categories").select("id").eq("special_id", category_special_id).single();
          if (cat) query = query.eq("category_id", cat.id);
        }
        const { data: sub } = await query.single();
        if (!sub) return res.status(404).json({ error: `لم يتم العثور على قسم فرعي بالرقم الخاص: ${subcategory_special_id}` });
      ...
      if (!sub_sub_category_id && sub_sub_category_special_id) {
        const { data: ss } = await supabase.from("sub_sub_categories").select("id").eq("special_id", sub_sub_category_special_id).eq("subcategory_id", subcategory_id).single();
  ...
  app.patch("/api/admin/categories/:id", async (req, res) => {
    try {
      ...
      if (name !== undefined) updateData.name = name;
      if (image_url !== undefined) updateData.image_url = image_url;
      if (special_id !== undefined) updateData.special_id = special_id || null;
  ...
  app.patch("/api/admin/subcategories/:id", async (req, res) => {
    try {
      ...
      if (special_id !== undefined) updateData.special_id = special_id || null;
      if (category_id !== undefined) updateData.category_id = category_id;
      const { data, error } = await supabase.from("subcategories").update(updateData).eq("id", req.params.id).select().single();
      if (error) throw error;
      res.json(data);
    } catch (e: any) { res.status(500).json({ error: e.message }); }
  });
  ...
  app.patch("/api/admin/sub-sub-categories/:id", async (req, res) => {
    try {
      ...
      if (special_id !== undefined) updateData.special_id = special_id || null;
      if (subcategory_id !== undefined) updateData.subcategory_id = subcategory_id;
      const { data, error } = await supabase.from("sub_sub_categories").update(updateData).eq("id", req.params.id).select().single();
      if (error) throw error;
      res.json(data);
    } catch (e: any) { res.status(500).json({ error: e.message }); }
  });
  ...
  app.patch("/api/admin/products/:id", async (req, res) => {
    try {
      const { name, description, image_url, price, price_per_unit, store_type, requires_input, min_quantity, available, subcategory_id, sub_sub_category_id, external_id } = req.body;
      const updateData: any = {};
      if (name !== undefined) updateData.name = name;
      if (description !== undefined) updateData.description = description;
      if (image_url !== undefined) updateData.image_url = image_url;
      if (price !== undefined) updateData.price = parseFloat(price) || 0;
      if (price_per_unit !== undefined) updateData.price_per_unit = parseFloat(price_per_unit) || 0;
      if (store_type !== undefined) updateData.store_type = store_type;
      if (requires_input !== undefined) updateData.requires_input = requires_input;
      if (min_quantity !== undefined) updateData.min_quantity = min_quantity ? parseInt(min_quantity) : null;
      if (available !== undefined) updateData.available = available;
      if (subcategory_id !== undefined) updateData.subcategory_id = subcategory_id;
      if (sub_sub_category_id !== undefined) updateData.sub_sub_category_id = sub_sub_category_id || null;
      if (external_id !== undefined) updateData.external_id = external_id || null;
  ...
  app.patch("/api/admin/payment-methods/:id", async (req, res) => {
    try {
      ...
      const { data, error } = await supabase.from("payment_methods").update(updateData).eq("id", req.params.id).select().single();
      if (error) throw error;
      res.json(data);
    } catch (e: any) { res.status(500).json({ error: e.message }); }
  });
  ...
  app.patch("/api/admin/banners/:id", async (req, res) => {
    try {
      ...
      if (image_url !== undefined) updateData.image_url = image_url;
      if (active !== undefined) updateData.active = active;
      const { data, error } = await supabase.from("banners").update(updateData).eq("id", req.params.id).select().single();
      if (error) throw error;
      res.json(data);
    } catch (e: any) { res.status(500).json({ error: e.message }); }
  });
  ...
  app.patch("/api/admin/offers/:id", async (req, res) => {
    try {
      ...
      if (image_url !== undefined) updateData.image_url = image_url;
      if (active !== undefined) updateData.active = active;
      const { data, error } = await supabase.from("offers").update(updateData).eq("id", req.params.id).select().single();
      if (error) throw error;
      res.json(data);
    } catch (e: any) { res.status(500).json({ error: e.message }); }
  });
    ...
    try {
      ...
      if (type === "payment-methods") {
        ...
        if (txCount && txCount.length > 0) {
          // يوجد معاملات مرتبطة - نعطّل بدل الحذف
      ...
      if (type === "categories") {
        ...
        if (subs && subs.length > 0) {
          ...
          if (subsubs && subsubs.length > 0) {
            ...
            // حذف المنتجات المرتبطة بالأقسام الفرعية الفرعية
            await supabase.from("products").delete().in("sub_sub_category_id", subsubIds);
            // حذف الأقسام الفرعية الفرعية
            await supabase.from("sub_sub_categories").delete().in("id", subsubIds);
          }
          // حذف المنتجات المرتبطة بالأقسام الفرعية
      ...
      if (type === "subcategories") {
        ...
        await supabase.from("products").delete().eq("subcategory_id", id);
      }

      // حذف المنتجات المرتبطة بالقسم الفرعي الفرعي
      if (type === "sub-sub-categories") {
        await supabase.from("products").delete().eq("sub_sub_category_id", id);
      }
