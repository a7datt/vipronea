...
async function startServer() {
  ...
  app.post("/api/auth/login", async (req, res) => {
    ...
    if (user) {
      ...
      if (isMatch) {
        ...
        const { data: fullStats } = await supabase.from("user_stats").select("*").eq("user_id", user.id).single();
  ...
  app.post("/api/orders", async (req, res) => {
    try {
      ...
      if (adminChatId && adminBot) {
        adminBot.sendMessage(adminChatId, `🔔 طلب جديد #ORD${order.id}\nالاسم: ${user.name}\nالرقم الشخصي: ${user.personal_number}\nProduct: ${product.name}\nTotal: ${total}\nData: ${JSON.stringify(extraData)}`);
      }
  ...
  app.post("/api/rewards/claim", async (req, res) => {
    try {
      ...
      if (stats.claimed_reward_index >= rewardIndex) return res.status(400).json({ error: "Reward already claimed" });
      if (rewardIndex > 0 && stats.claimed_reward_index < rewardIndex - 1) return res.status(400).json({ error: "Claim previous rewards first" });

      const goals = [5, 15, 30, 50, 100, 200, 500];
      if (stats.total_recharge_sum < goals[rewardIndex]) return res.status(400).json({ error: "Goal not reached yet" });

      await applyReward(userId, rewardIndex);
      await supabase.from("notifications").insert({
        user_id: userId,
        title: "تم استلام مكافأة",
        message: `لقد حصلت على مكافأة هدف ${goals[rewardIndex]}$!`,
        type: "success"
      });
  ...
  app.post("/api/admin/categories", async (req, res) => {
    try {
      ...
      const { data, error } = await supabase.from("categories").insert({ name, image_url, order_index, active, special_id }).select().single();
      if (error) throw error;
      res.json(data);
    } catch (e: any) {
      res.status(500).json({ error: e.message });
  ...
  app.post("/api/admin/subcategories", async (req, res) => {
    try {
      const { category_id, name, image_url, order_index, active, special_id } = req.body;
      const { data, error } = await supabase.from("subcategories").insert({ category_id, name, image_url, order_index, active, special_id }).select().single();
      if (error) throw error;
      res.json(data);
    } catch (e: any) {
      res.status(500).json({ error: e.message });
  ...
  app.post("/api/admin/sub-sub-categories", async (req, res) => {
    try {
      const { subcategory_id, name, image_url, order_index, active, special_id } = req.body;
      const { data, error } = await supabase.from("sub_sub_categories").insert({ subcategory_id, name, image_url, order_index, active, special_id }).select().single();
      if (error) throw error;
      res.json(data);
    } catch (e: any) {
      res.status(500).json({ error: e.message });
  ...
  app.post("/api/admin/products", async (req, res) => {
    try {
      const { subcategory_id, sub_sub_category_id, name, price, description, image_url, store_type, requires_input, min_quantity, available, external_id } = req.body;
      const { data, error } = await supabase.from("products").insert({
        subcategory_id, sub_sub_category_id, name, price, description, image_url, store_type, requires_input, min_quantity, available, external_id
      }).select().single();
      if (error) throw error;
      res.json(data);
    } catch (e: any) {
  ...
  app.get("/api/admin/chat/list", async (req, res) => {
    try {
      const { data: msgs } = await supabase.from("messages").select("user_id, guest_id, content, created_at, sender_role, is_read").order("created_at", { ascending: false });
      res.json(msgs || []);
    } catch (e: any) {
      res.status(500).json({ error: e.message });
    }
...
async function applyReward(userId: string, goalIndex: number) {
  ...
  if (goalIndex === 0) {
    ...
    await supabase.rpc("increment_balance", { user_id_param: userId, amount_param: 1 });
    await supabase.from("user_stats").update({ active_discount: 2, discount_expires_at: oneMonth }).eq("user_id", userId);
  } else if (goalIndex === 2) {
    await supabase.rpc("increment_balance", { user_id_param: userId, amount_param: 3 });
    ...
    await supabase.rpc("increment_balance", { user_id_param: userId, amount_param: 5 });
    ...
    await supabase.rpc("increment_balance", { user_id_param: userId, amount_param: 10 });
    ...
    await supabase.rpc("increment_balance", { user_id_param: userId, amount_param: 20 });
    ...
    try {
      ...
      adminBot.onText(/\/start/, (msg) => {
        ...
        const welcomeMsg =
          ...
          `🗑️ الحذف:\n/delcat /delsub /delsubsub /delprod /delbanner /deloffer /delvoucher`;
      ...
      adminBot.onText(/\/sub/, (msg) => {
        ...
        adminBot!.sendMessage(msg.chat.id, "📂 يرجى إدخال ID القسم الرئيسي:");
      });

      adminBot.onText(/\/subsub/, (msg) => {
        if (msg.chat.id.toString() !== adminChatId) return;
        userStates.set(msg.chat.id, { step: "admin_add_subsub_subid", data: {} });
        adminBot!.sendMessage(msg.chat.id, "📂 يرجى إدخال ID القسم الفرعي:");
      });
      ...
      adminBot.onText(/\/delcat/, (msg) => {
        if (msg.chat.id.toString() !== adminChatId) return;
        userStates.set(msg.chat.id, { step: "admin_del_cat_id", data: {} });
        adminBot!.sendMessage(msg.chat.id, "🗑️ يرجى إدخال ID القسم المراد حذفه:");
      });

      adminBot.onText(/\/delsub/, (msg) => {
        if (msg.chat.id.toString() !== adminChatId) return;
        userStates.set(msg.chat.id, { step: "admin_del_sub_id", data: {} });
        adminBot!.sendMessage(msg.chat.id, "🗑️ يرجى إدخال ID القسم الفرعي المراد حذفه:");
      });

      adminBot.onText(/\/delsubsub/, (msg) => {
        if (msg.chat.id.toString() !== adminChatId) return;
        userStates.set(msg.chat.id, { step: "admin_del_subsub_id", data: {} });
        adminBot!.sendMessage(msg.chat.id, "🗑️ يرجى إدخال ID القسم الفرعي الفرعي المراد حذفه:");
      });

      adminBot.onText(/\/delprod/, (msg) => {
        if (msg.chat.id.toString() !== adminChatId) return;
        userStates.set(msg.chat.id, { step: "admin_del_prod_id", data: {} });
        adminBot!.sendMessage(msg.chat.id, "🗑️ يرجى إدخال ID المنتج المراد حذفه:");
      });

      adminBot.onText(/\/delbanner/, (msg) => {
        if (msg.chat.id.toString() !== adminChatId) return;
        userStates.set(msg.chat.id, { step: "admin_del_banner_id", data: {} });
        adminBot!.sendMessage(msg.chat.id, "🗑️ يرجى إدخال ID البانر المراد حذفه:");
      });

      adminBot.onText(/\/deloffer/, (msg) => {
        if (msg.chat.id.toString() !== adminChatId) return;
        userStates.set(msg.chat.id, { step: "admin_del_offer_id", data: {} });
        adminBot!.sendMessage(msg.chat.id, "🗑️ يرجى إدخال ID العرض المراد حذفه:");
      });

      adminBot.onText(/\/delvoucher/, (msg) => {
        if (msg.chat.id.toString() !== adminChatId) return;
        userStates.set(msg.chat.id, { step: "admin_del_voucher_id", data: {} });
        adminBot!.sendMessage(msg.chat.id, "🗑️ يرجى إدخال ID القسيمة المراد حذفها:");
      });

      // Admin callback_query: Approve/Reject transactions & orders
      ...
      adminBot.on("message", async (msg: any) => {
        ...
        if (state.step === "admin_broadcast_msg") {
          ...
          if (text === "نعم") {
            await supabase.from("users").delete().eq("id", state.data.userId);
            adminBot!.sendMessage(chatId, `✅ تم حذف المستخدم ${state.data.userName} نهائياً.`);
          } else {
          ...
          const { data: cat } = await supabase.from("categories").insert({ name: state.data.name, image_url: text }).select().single();
          userStates.delete(chatId);
          ...
          const { data: sub } = await supabase.from("subcategories").insert({ category_id: state.data.catId, name: state.data.name, image_url: text }).select().single();
          userStates.delete(chatId);
          ...
          const { data: ss } = await supabase.from("sub_sub_categories").insert({ subcategory_id: state.data.subId, name: state.data.name, image_url: text }).select().single();
          userStates.delete(chatId);
