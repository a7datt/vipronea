...
async function startServer() {
  ...
  app.post("/api/admin/subcategories", async (req, res) => {
    try {
      ...
      if (!category_id && category_special_id) {
        ...
        if (!cat) return res.status(404).json({ error: `لم يتم العثور على قسم رئيسي بالرقم الخاص: ${category_special_id}` });
        category_id = cat.id;
      }
  ...
  app.post("/api/admin/sub-sub-categories", async (req, res) => {
    try {
      ...
      // إذا أرسلت الواجهة subcategory_special_id نبحث عن الـ id الحقيقي
      if (!subcategory_id && subcategory_special_id) {
        const { data: sub } = await supabase.from("subcategories").select("id").eq("special_id", subcategory_special_id).single();
