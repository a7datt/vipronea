...
export default function App() {
  ...
  useEffect(() => {
    const channels = [
      ...
      supabase.channel('settings-changes').on('postgres_changes', { event: '*', schema: 'public', table: 'settings' }, () => window.location.reload()).subscribe(),
    ];
...
const AdminPanel = ({
  ...
  const [adminOrders, setAdminOrders] = useState<any[]>([]);
    ...
    useEffect(() => {
      ...
      fetchAdminSettings();
      fetchAdminVouchers();
      // جلب كل الأقسام بدون فلتر لعرضها في لوحة التحكم
      fetchCategories();
      fetchSubcategories(); // بدون catId = كل الأقسام الفرعية
      fetchSubSubCategories(); // بدون subId = كل الأقسام الفرعية الفرعية
      fetchPaymentMethods();
      fetchBanners();
      fetchOffers();
    }, []);
